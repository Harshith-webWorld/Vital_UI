...
function NewReportGraph({
  ...
  const { data, options } = config == 'mf' ? mfconfigs(chart.labels || [], chart.datasets || {}, chart.displayY || false, chart.showunit || '') : configs(chart.labels || [], chart.datasets || {}, chart.displayY || false, chart.showunit || '');
          ...
          <Box style={{ height: "250px", marginTop: 30 }}>
            <Line
              ...
              getElementAtEvent={i => {
                ...
                if (i[0]?._chart?.config?.data?.datasets[0]?.label === "MF Rate") {
                  console.log("in")
