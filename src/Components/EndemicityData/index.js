...
const EndemicityData = (props) => {
  ...
  async function getEndemicityMapAllDistricts() {
    ...
    if ((endemicityData && endemicityData.length > 0) && (geoData && geoData.length > 0)) {
      ...
      endemicityData.map((ele) => {
        updatedDistrictData.map((districtEle) => {
          if (ele.mapDistrictId == districtEle.properties.DIST_CODE) {
            ...
            districtEle.properties.NoOfLFCases > 0 && lfcasesAvg.push(districtEle.properties.NoOfLFCases);
            districtEle.properties.NoOfHydroceleCases > 0 && hydroceleCasesAvg.push(districtEle.properties.NoOfHydroceleCases);
            districtEle.properties.NoMFPosetive > 0 && mfPositiveAvg.push(districtEle.properties.NoMFPosetive);
            districtEle.properties.CulexInfection > 0 && infectionAvg.push(districtEle.properties.CulexInfection);
            districtEle.properties.CulexInfectivity > 0 && infectivityAvg.push(districtEle.properties.CulexInfectivity);

            (districtEle.properties.NoOfLFCases + districtEle.properties.NoOfHydroceleCases + districtEle.properties.NoMFPosetive) > 0 &&
              endemicityAvg.push((districtEle.properties.NoOfLFCases + districtEle.properties.NoOfHydroceleCases + districtEle.properties.NoMFPosetive));
          }
        })
      })
      ...
      setTimeout(() => {
        setDistrictMap(<GeoJSON key={`geojson-01`} style={districtStyle} data={updatedDistrictData} onEachFeature={onEachDistrict} cursor={false} dragging={false} zoomControl={false} scrollWheelZoom={false} doubleClickZoom={false} boxZoom={false} />)
      }, 500);
    }
  }
  ...
  const onEachDistrict = (district, layer) => {
    ...
    if (props.endemicityProps === "LfCases") {
      ...
      endemicity = district.properties.NoOfLFCases + district.properties.NoOfHydroceleCases + district.properties.NoMFPosetive;
    ...
    layer.bindTooltip("" + endemicity + "", { permanent: true, direction: "center" }).openTooltip()
    layer.on({
      mouseover: (e) => {
        e.target.setStyle({
          color: "Black",
        ...
        if (props.endemicityProps === "LfCases") {
          e.target.bindPopup(`District: ${districtName} , LF: ${endemicity}`).openPopup();
        } else if (props.endemicityProps === "HydroceleCases") {
          e.target.bindPopup(`District: ${districtName} , Hydrocele: ${endemicity} - Surg.(${surg})`).openPopup();
        } else if (props.endemicityProps === "MfPositive") {
          e.target.bindPopup(`District: ${districtName} , MF: ${endemicity}`).openPopup();
        } else if (props.endemicityProps === "CulexInfectivity") {
          e.target.bindPopup(`District: ${districtName} , Infectivity: ${endemicity}`).openPopup();
        } else if (props.endemicityProps === "CulexInfection") {
          e.target.bindPopup(`District: ${districtName} , Infection: ${endemicity}`).openPopup();
        } else {
          e.target.bindPopup(`District: ${districtName} , Endemicity: ${endemicity} , LF: ${district.properties.NoOfLFCases} , HC: ${district.properties.NoOfHydroceleCases} , MF: ${district.properties.NoMFPosetive}`).openPopup();
        }
      },
      mouseout: (e) => {
        e.target.setStyle({
  ...
  }, [endemicityData, geoData]); //,[geoData]
  useEffect(() => { setendemicityData(props.endemicityData) }, [props.endemicityData])
  useEffect(() => { setGeoData(props.geoData) }, [props.geoData])
  return (
    <div style={{ zoom: "1" }}>
      <MapContainer style={{ height: props.mapContainerHeight }} zoom={props.zoomSize} center={[19, 76]} whenCreated={setMap} ref={geoRef} cursor={false} dragging={true} zoomControl={false} scrollWheelZoom={false} doubleClickZoom={true} boxZoom={false} trackResize={true} >
        {districtMap}
        {props.reset == 'false' ? null : <MapReset />}
      </MapContainer>
    </div>
  )
