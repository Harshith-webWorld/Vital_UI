...
// import { reportTableStyle } from '../../../NewReports/utils';
...
const PhcWiseDrugConsumption = () => {
  ...
  useEffect(() => {
    ...
    getMdaphcWiseDrugConsumption(data);
  }, [initialFilterValue]);

  async function getMdaphcWiseDrugConsumption(filterData) {
    const reportData: any = await ReportService.getMdaphcWiseDrugConsumption(
      filterData
    );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getMdaphcWiseDrugConsumption(values);
    },
    onReset: () => {
  ...
  const createpdf = () => {
    ...
    if (input) {
      html2canvas(input).then((canvas: any) => {
        ...
        doc.save('Drug Consumption PHC Wise.pdf');
      });
    }
  };
  ...
  `}</style>
      ...
      >
        <div style={{ display: 'flex', marginTop: '2%', marginBottom: '2%' }}>
          <p className='rpthead'>Drug Consumption PHC Wise</p>
          <div
            style={{
              display: 'flex',
              marginLeft: 'auto',
          ...
          >
            ...
            <ReactHTMLTableToExcel
              ...
              filename=' Drug Consumption PHC Wise'
              sheet=' Drug Consumption PHC Wise'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                ...
                <tr>
                  ...
                  <td>No. of eligible population</td>
                  <td>No. of people administered</td>
                </tr>
              </thead>
              <tbody>
                    ...
                    .map((data: any, index: any) => {
                      return (
                        <tr key={index}>
                          ...
                          <td align='right'>{data.noOfPeopleAdministered}</td>
                        </tr>
                      );
                    })
                ) : (
...
export default PhcWiseDrugConsumption;
