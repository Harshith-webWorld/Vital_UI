...
const VspAvailabiltyConsumptionLabMaterials = () => {
  ...
  useEffect(() => {
    ...
    getVspAvailabilityConsumptionLabMaterials(data);
  }, [initialFilterValue]);

  async function getVspAvailabilityConsumptionLabMaterials(filterData) {
    const reportData: any =
      await ReportService.getVspAvailabilityConsumptionLabMaterials(filterData);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getVspAvailabilityConsumptionLabMaterials(values);
    },
    onReset: () => {
  ...
  const createpdf = () => {
    ...
    if (input) {
      html2canvas(input).then((canvas: any) => {
        ...
        doc.save('Vertical Unit Availability Consumption lab materials.pdf');
      });
    }
  };
          ...
          >
            <button
              ...
              onClick={() =>
                Exportpdf(
                  'table-to-xlsnew',
                  `Vertical Unit Availability Consumption lab materials.pdf`,
                  'p',
                  210,
                  295
                )
            ...
            <ReactHTMLTableToExcel
              ...
              filename=' Vertical Unit Availability Consumption lab materials'
              sheet=' Vertical Unit Availability Consumption lab materials'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                <tr>
                  ...
                  <td>Items</td>
                  <td>Opening</td>
                  <td>Received During Month</td>

                  <td>Received From District</td>
                  <td>Others From Whom Received</td>
                  <td>From Whom Received</td>
                  <td>Total</td>
                  <td>
                    Actual Consumption During
                    <br /> the Month
                  </td>
                  <td>Issue to Other in Quantity</td>
                  <td>To Whom Issued</td>
                  <td>
                    Balance at the
                    <br /> End of Month
                  </td>
                  ...
                  reportTableData
                    ...
                    .map((data: any, index: any) => (
                      <tr key={`index+${index}`}>
                        ...
                        <td>{data.unitName}</td>
                        <td>{data.itemName}</td>
                        <td align='right'>{data.openingBalanceQty}</td>
                        <td align='right'>{data.receivedDuringMonthQty}</td>
                        <td align='right'>{data.receivedFromDistrictName}</td>
                        <td align='right'>{data.receivedFromWhomOthers}</td>
                        <td>{data.receivedFromWhom}</td>
                        <td align='right'>{data.totalStock}</td>
                        <td align='right'>{data.actualConsumption}</td>
                        <td align='right'>{data.issueToOtherQty}</td>
                        <td>{data.issuedToWhom}</td>
                        <td align='right'>{data.balanceEndOfMonth}</td>
                        <td align='right'>{data.reqNext3MonthsQty}</td>
                      </tr>
                    ))
                ) : (
                  <tr>
...
export default VspAvailabiltyConsumptionLabMaterials;
