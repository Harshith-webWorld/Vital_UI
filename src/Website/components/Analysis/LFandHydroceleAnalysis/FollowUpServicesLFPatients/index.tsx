...
const FollowUpServicesLFandHF = () => {
  ...
  useEffect(() => {
    ...
    getFollowUpServicesLFPatients(data);
  }, [initialFilterValue]);

  async function getFollowUpServicesLFPatients(filterData) {
    const reportData: any = await ReportService.getFollowUpServicesLFPatients(
      filterData
    );

    console.log(reportData.serviceMMDPKitGivenDate, 'date');

    // if (reportData.data.isServiceMMDPKitGiven == false) {
    //   reportData.data.serviceMMDPKitGivenDate = 'No';
    // }
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getFollowUpServicesLFPatients(values);
    },
    onReset: () => {
  ...
  `}</style>
      ...
      >
        <div style={{ display: 'flex', marginTop: '2%', marginBottom: '2%' }}>
          <p className='rpthead'>Follow Up services LF Patients</p>
          <div
            style={{
              display: 'flex',
              marginLeft: 'auto',
          ...
          >
            <button
              ...
              onClick={() =>
                Exportpdf(
                  'table-to-xlsnew',
                  `Follow Up services LF Patients.pdf`,
                  'p',
                  210,
                  295
                )
            ...
            <ReactHTMLTableToExcel
              ...
              filename='Follow Up services LF Patients'
              sheet='Follow Up services LF Patients'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                ...
                <tr>
                  ...
                  <td rowSpan={2}>Subcenter</td>
                  <td rowSpan={2}>Year</td>
                  <td rowSpan={2}>Month</td>
                  <td colSpan={2}>Details of Service Provider</td>
                  <td colSpan={9}>Follow Up</td>
                </tr>
                <tr>
                  <td>Name</td>
                  <td>Date Of Visit</td>

                  <td>Is MMDP Training Given</td>
                  <td>
                    Date of MMDP <br /> Training Given
                  </td>
                  <td>Is MM Following</td>
                  <td>Date of MM Followed</td>
                  <td>Is MMDP Kit Given</td>
                  <td>
                    Date of MMDP
                    <br /> Kit Given
                  </td>
                  <td>Is Medicine Given</td>
                  <td>Date of Medicine Given</td>
                  <td>Follow up Lost reason</td>
                </tr>
              </thead>
              <tbody>
                    ...
                    .map((data: any, index: any) => {
                      return (
                        <tr key={`FUSLFP${index}`}>
                          ...
                          <td align='right'>{data.serviceDateOfVisit}</td>
                          <td>
                            {data.isServiceMMDPTrainingGiven ? 'Yes' : 'No'}
                          </td>
                          <td align='right'>
                            {data.isServiceMMDPTrainingGiven
                              ? data.serviceMMDPTrainingDate
                              : ''}
                          </td>
                          <td>
                            {data.isServicePatientFollowingMM ? 'Yes' : 'No'}
                          </td>
                          <td align='right'>
                            {data.isServicePatientFollowingMM
                              ? data.servicePatientFollowingDate
                              : ''}
                          </td>
                          <td>{data.isServiceMMDPKitGiven ? 'Yes' : 'No'}</td>
                          <td align='right'>
                            {data.isServiceMMDPKitGiven
                              ? data.serviceMMDPKitGivenDate
                              : ''}
                          </td>
                          <td>{data.isServiceMedicineGiven ? 'Yes' : 'No'}</td>
                          <td align='right'>
                            {data.isServiceMedicineGiven
                              ? data.serviceMedicineGivenDate
                              : ''}
                          </td>
                          <td>{data.followUpLostReason}</td>
                        </tr>
                      );
                    })
                ) : (
...
export default FollowUpServicesLFandHF;
