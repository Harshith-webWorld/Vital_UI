...
const VspMonthlyVacancyStatus = () => {
  ...
  useEffect(() => {
    ...
    getVspMonthlyVacancyStatus(data);
  }, [initialFilterValue]);

  async function getVspMonthlyVacancyStatus(filterData) {
    const reportData: any = await ReportService.getVspMonthlyVacancyStatus(
      filterData
    );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getVspMonthlyVacancyStatus(values);
    },
    onReset: () => {
  ...
  const createpdf = () => {
    ...
    if (input) {
      html2canvas(input).then((canvas: any) => {
        ...
        doc.save('Vertical Unit Monthly Vacancy Status.pdf');
      });
    }
  };
  ...
  `}</style>
      ...
      >
        <div style={{ display: 'flex', marginTop: '2%', marginBottom: '2%' }}>
          <p className='rpthead'>Vertical Unit Monthly Vacancy Status</p>
          <div
            style={{
              display: 'flex',
              marginLeft: 'auto',
          ...
          >
            <button
              ...
              onClick={() =>
                Exportpdf(
                  'table-to-xlsnew',
                  `Vertical Unit Monthly Vacancy Status.pdf`,
                  'p',
                  210,
                  295
                )
            ...
            <ReactHTMLTableToExcel
              ...
              filename=' Vertical Unit Monthly Vacancy Status'
              sheet=' Vertical Unit Monthly Vacancy Status'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                <tr>
                  ...
                  <td>No. of staff filled</td>
                  <td>No. of staff vacant</td>
                </tr>
              </thead>
              <tbody>
                    ...
                    .map((data: any, index: any) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>{data.cadre}</td>
                          <td align='right'>{data.cadreOther}</td>
                          <td align='right'>{data.sanctioned}</td>
                          <td align='right'>{data.filled}</td>
                          <td align='right'>{data.vacant}</td>
                        </tr>
                      );
                    })
                ) : (
...
export default VspMonthlyVacancyStatus;
