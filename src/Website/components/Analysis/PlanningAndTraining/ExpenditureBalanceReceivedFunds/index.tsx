...
const ExpenditureBalanceReceivedFunds = () => {
  ...
  useEffect(() => {
    ...
    getExpenditureBalanceReceivedFunds(data);
  }, [initialFilterValue]);

  async function getExpenditureBalanceReceivedFunds(filterData) {
    const reportData: any =
      await ReportService.getExpenditureBalanceReceivedFunds(filterData);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getExpenditureBalanceReceivedFunds(values);
    },
    onReset: () => {
  ...
  const createpdf = () => {
    ...
    if (input) {
      html2canvas(input).then((canvas: any) => {
        ...
        doc.save('Expenditure Balance Received Funds.pdf');
      });
    }
  };
  ...
  `}</style>
      ...
      >
        <div style={{ display: 'flex', marginTop: '2%', marginBottom: '2%' }}>
          <p className='rpthead'>Expenditure Balance Received Funds</p>
          <div
            style={{
              display: 'flex',
              marginLeft: 'auto',
          ...
          >
            <button
              ...
              onClick={() =>
                Exportpdf(
                  'table-to-xlsnew',
                  `Expenditure Balance Received Funds.pdf`,
                  'p',
                  210,
                  295
                )
            ...
            <ReactHTMLTableToExcel
              ...
              filename=' Expenditure Balance Received Funds'
              sheet=' Expenditure Balance Received Funds'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                <tr>
                  <td>Sr.No.</td>
                  <td>District</td>
                  <td>Name Of Material activity</td>

                  <td>No. of Material activity</td>

                  <td>
                    Material activity cost
                    <br /> in Rs
                  </td>
                  <td>Statement of funds allotted</td>
                  <td>
                    Date of District
                    <br /> Coordition Committe
                  </td>
                  <td>Fund Allocated With Date</td>

                  <td>Fund Utilised With Date</td>
                  <td>Fund Utilised With Date</td>
                </tr>
              </thead>
              <tbody>
                  ...
                  reportTableData
                    ...
                    .map((data: any, index: any) => (
                      <tr>
                        ...
                        <td>{data.materialActivity}</td>
                        <td align='right'>{data.materialActivityNo}</td>
                        <td align='right'>{data.materialActivityCostInRs}</td>
                        <td>{data.statementOfFundsAllotted}</td>
                        <td align='right'>{data.dateDistrictCoordComitte}</td>
                        <td align='right'>{data.fundAllocatedWithDate}</td>
                        <td align='right'>{data.fundUtilisedWithDate}</td>
                        <td align='right'>{data.fundBalanceAfterRound}</td>
                      </tr>
                    ))
                ) : (
                  <tr>
...
export default ExpenditureBalanceReceivedFunds;
