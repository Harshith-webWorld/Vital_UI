...
const LarvicidalReport1 = (props) => {
  ...
  useEffect(() => {
    ...
    getEntologyLaravicidalReport1(data);
  }, [initialFilterValue]);

  async function getEntologyLaravicidalReport1(filterData) {
    const reportData: any = await ReportService.getEntologyLaravicidalReport1(
      filterData
    );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getEntologyLaravicidalReport1(values);
    },
    onReset: () => {
      ...
      getEntologyLaravicidalReport1(initialFilterValue());
    },
  });
  ...
  `}</style>
      ...
      >
        <div style={{ display: 'flex', marginTop: '2%', marginBottom: '2%' }}>
          <p className='rpthead'>Larvicidal Treatment</p>
          <div
            style={{
              display: 'flex',
              marginLeft: 'auto',
          ...
          >
            ...
            <ReactHTMLTableToExcel
              ...
              filename='Larvicidal Treatment'
              sheet='Larvicidal Treatment'
            ...
            <table id='table-to-xlsnew'>
              <thead>
                <tr>
                  ...
                  <td rowSpan={2}> Sub Unit/A type Unit</td>
                  <td rowSpan={2}> Larvicide</td>
                  <td colSpan={2}> No. of men working</td>
                  <td rowSpan={2}> Opening Balance in liters/kg</td>
                  <td rowSpan={2}> Received During the Month</td>
                  <td rowSpan={2}> Consumed During the Month </td>
                  <td rowSpan={2}> Balance at end of Month</td>
                  <td colSpan={4}> Minor Engg. Work in approx. Sq. meters </td>
                  <td rowSpan={2}> Other</td>
                  <td colSpan={3}> Biological Control</td>
                </tr>
                <tr>
                  <td>SFW</td>
                  <td>FW</td>
                ...
                {reportTableData && reportTableData.length > 0 ? (
                  reportTableData
                    ...
                    .map((data: any, index: any) => (
                      <tr>
                        <td>{index + 1}</td>
                        <td>{data.year === 0 ? '' : data.year}</td>
                        <td>{monthAlphabetGenerator(data.month)}</td>
                        <td>{data.districtName}</td>
                        <td>{data.nameOfControlUnit}</td>
                        <td>{''}</td>
                        <td>{data.larvicideName}</td>
                        <td align='right'>{data.noOfMenWorkingSFW}</td>
                        <td align='right'>{data.noOfMenWorkingFW}</td>
                        <td align='right'>{data.openingBalance}</td>
                        <td align='right'>{data.receivedDuringMonth}</td>
                        <td align='right'>{data.consumedDuringMonth}</td>
                        <td align='right'>{data.balanceEndOfMonth}</td>
                        <td align='right'>{data.canalisationWork}</td>
                        <td align='right'>{data.desiltingWork}</td>
                        <td align='right'>{data.deweedingWork}</td>
                        <td align='right'>{data.fillingWork}</td>
                        <td align='right'>{data.otherWork}</td>
                        <td align='right'>{data.noOfWellsBioControl}</td>
                        <td align='right'>{data.noOfTankBioControl}</td>
                        <td align='right'>{data.noOfCanalsBioControl}</td>
                      </tr>
                    ))
                ) : (
                  <tr>
...
export default LarvicidalReport1;
