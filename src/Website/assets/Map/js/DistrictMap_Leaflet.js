...
DistrictMap_Leaflet.prototype = {
  ...
  setup_map: function () {
    ...
    $("#village-list").on("change", function (val) {
      ...
      self.village_name = $("#village-list").val();
      self.mapnavigate(self.village_name);
    });
  },
  ...
  updateLegend: function (max) {
    ...
    if (config.categorical_variables.indexOf(this.field_name) !== -1) {
  ...
  //   else if (!F) return N;
  //   else if (F >= 10000000)
  //     return Math.round((100 * F) / 10000000) / 100 + " Cr";
  ...
  create_map: function () {
    ...
    this.parseCSVFile().done(function (result) {
      ...
      $.ajax({
        url: "geometry/" + config.geojson_file_map[self.district_name] + ".geojson",
  ...
  setFieldName: function (name) {
    ...
    //this.loading(true);
    this.field_name =
      self.isCustom === true ? name : config.fields_short_long_map[name];
