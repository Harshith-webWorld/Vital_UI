...
const VerticalControlUnit = () => {
  ...
  async function getStateList() {
    const response = await verticalControlUnitService.getState();
  ...
  async function gettalukaList() {
    const response = await verticalControlUnitService.gettaluka();
    if (response && response.data) {
  ...
  async function getDistrictInfoForTaluka() {
    const response = await verticalControlUnitService.getDistrict();
    if (response && response.data) {
  ...
  async function getVillageList() {
    const response = await verticalControlUnitService.getVillage();
    if (response && response.data) {
  ...
  async function getVerticalControlUnit() {
    const response = await verticalControlUnitService.getVerticalControlUnit();
    if (response && response.data) {
      setVerticalControlUnitList(response.data);
    }
  }
  ...
  const districtExtractor = (districtList, id) => {
    ...
    return result.length === 0 ? "-" : result[0].districtName;
  };
  const talukaExtractor = (talukaList, id) => {
    ...
    return result.length === 0 ? "-" : result[0].talukaName;
  };
  const nameOfOfficerExtractor = (nameOfOfficer) => {
    var result = nameOfOfficer ? nameOfOfficer : "-";
    return result;
  }
  const columns = [
    {
      name: "Vertical Control Unit Name",
      selector: "nameOfControlUnit",
    },
    {
      name: 'Unit Type',
      selector: 'unitType',
    },
    {
      ...
      selector: (row: any) => (
        <div>{nameOfOfficerExtractor(row.nameOfOfficer)}</div>
      ),
    },
    {
      name: "Taluka",
        ...
        item.isActive ? (
          <>
            <button
              className="btn"
              onClick={() =>
                history.push("/VerticalControlUnitForm", { id: item.id, view: "viewOnly", unitType: item.unitType })
              }
            >
            ...
            <button
              ...
              onClick={() => history.push("/VerticalControlUnitForm", { id: item.id, unitType: item.unitType })}
            >
          ...
          <div className="card-header">
            <div className="row">
              <div className="col-md-9">
                <h5 className="cardtitlenew">Vertical Control Units</h5>
              </div>
...
export default VerticalControlUnit;
